...
#move the robot: right left or center
def move_main_direction(direction,rotation):
    move_arm(direction.get_position(), direction.get_quaternion(), True)
    rotate_arm(rotation)
    return direction

#kick in right side
def kick_in_right(ymain):
...
for game in range(num_games):
    ...
    x1=Frame_ballCoordinates()[0]
    y1=Frame_ballCoordinates()[1]
    agent.set_joint_positions(start_position)
...
pr.stop()
pr.shutdown()
with open('test_policy.csv', 'a', newline='') as file:

...
for game in range(num_games):
    ...
    def moving_to_direction():
        ...
        if ymain<2 and ymain>=0.6:
            move_arm(right.get_position(),right.get_quaternion(), True)
            rotate_arm(-pi/6)
            ...
            move_arm(left.get_position(), left.get_quaternion(), True)
            rotate_arm(pi / 6)
            main_direction=left
    ...
    if isinstance(ball_temp,str):
        ...
        if ball_position is None or touch_wall or (robot_position[0]>ball_position[0]):
            ...
            temp.append(1)
    initial_positions.append(temp)
pr.stop()
pr.shutdown()
with open('policy_records.csv', 'a', newline='') as file:
